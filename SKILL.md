...
description: >
  ...
  5. If user mentions 黑名单/屏蔽/block/unblock/不想看 → blacklist management mode.
  6. If user mentions 收藏/喜欢这张/favorite/bookmark/取消收藏/unfavorite → favorites mode.
  7. If cron-triggered → exploration mode per EXPLORER.md.
...
执行:`exec bash {baseDir}/scripts/setup.sh`

这个脚本会创建 `wanderclaw/` 目录、拷贝模板文件（state.json / interest-graph.json / postcards.json / postcard-index.json / sources.yaml / EXPLORER.md / SOUL.md / postcard-format.md），且对已存在的文件不覆盖。

**初始化后**,写入兴趣到 interest-graph.json,写入 search_tool 到 state.json。
...
这个脚本会注册 4 个日常探索 cron(09:00/12:00/15:00/20:00)+ 深潜 + 周度总结。
